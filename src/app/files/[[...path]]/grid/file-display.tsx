...
export function FileDisplay({ file }: { file: FileData}) {
    ...
    return (
        <div className=" items-center rounded-lg bg-white/10 p-2 pb-0 hover:bg-white/20 w-[70px] h-[100px]" >
            <Link href={"/files/" + file.path}>
            <div className="items-center justify-center rounded-lg bg-white/10 p-1 ">
                <Image src={"/icons/" + icon} width={60} height={60} alt={file.name} />
            </div>
            <div className="text-xs text-center h-[38px] break-words items-center p-1">
...
function getIcon(fileData: FileData) {
    return (fileData.type === FileType.File) ? getIconForFile(fileData.name) : getIconForFolder(fileData.name)
}

function DownloadDialog({ file, open, setOpen }: { file: FileData, open: boolean, setOpen: (arg0: boolean) => void}) {
